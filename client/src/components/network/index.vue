                    ...
                    <v-card-text>
                        <v-container grid-list-md>
                            <v-layout wrap>
                                ...
                                <v-flex xs12 sm6 md6>
                                    <date-field :default-date="editedItem.time_created | toIso" :label="'Time created'" @change="updateDate" ref="date1"></date-field>
                                </v-flex>
                                <v-flex xs12 sm6 md6>
                                    <v-select
                                ...
                                <v-flex xs12 sm6 md6>
                                    <date-field :default-date="editedItem.last_successful_connection | toIso" :label="'Last successful connection'" @change="updateDate" ref="date2"></date-field>
                                </v-flex>
                            </v-layout>
                        </v-container>
                    </v-card-text>
            ...
            <v-expansion-panel-content v-for="(record,i) in records" :key="i" header>
                ...
                <v-card>
                    <v-container grid-list-md text-xs-center>
                        <v-layout row wrap>
                            <v-flex v-for="(rec, subject) in {'Type':record.network_type,'Time created':record.time_created,'Last successful connection':record.last_successful_connection,'Action':'Edit'}"
                                    :key="subject" xs3>
                                <template v-if="subject === 'Action'">
                                        <v-card-text class="font-weight-bold text-xs-right text-uppercase">Action</v-card-text>
                                        <v-card-text class="text-xs-right ">
                                            <v-icon small class="mr-2" color="blue" @click="editItem(record)">edit
                                            </v-icon>
                                            <v-icon small @click="deleteItem(record)" color="red">delete</v-icon>
                                        </v-card-text>
                                </template>
                                <template v-else-if="isDateField(subject)">
                                    <v-card-text class="px-0 font-weight-bold text-uppercase">
                                        {{subject}}
                                    </v-card-text>
                                    <v-card-text>
                                        {{ rec | toIso }}
    ...
    export default {
        ...
        data: () => ({
            ...
            headers: [
                ...
                {text: 'Last Successful Connection', align: 'left', value: 'last_successful_connection'},
            ],
            editedIndex: -1,
            editedItem: {
                id: '',
        ...
        methods: {
            ...
            save() {
                if (this.editedIndex > -1) {
                    ...
                    this.saveCurrentModel(this.editedItem);
                } else {
            ...
            updateDefaultDate(){
                this.$refs.date1.updateDefaultDate(this.$options.filters.toIso(this.editedItem.time_created));
                this.$refs.date2.updateDefaultDate(this.$options.filters.toIso(this.editedItem.last_successful_connection));
            },
            createItem(){
